...
import { chordsInScale } from "@/lib/engine/scale-harmony";
...
export default function ScalesPage() {
  ...
  // El número va en la propia pestaña: es la forma de que se vea que ahí hay
  // una lista completa esperando, sin tener que entrar a descubrirlo.
  const chordCount = useMemo(
    () => chordsInScale(scale).reduce((total, group) => total + group.chords.length, 0),
    [scale],
  );
      ...
      <div className="no-print flex flex-wrap gap-1 border-b border-stone-200">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            {t.id === "acordes" && (
              <span className="ml-1.5 text-xs text-stone-400">{chordCount}</span>
            )}
          </button>
        ))}
      </div>
